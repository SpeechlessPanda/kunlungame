# Delta 2026-04-24：Galgame 昆仑子人格 + 结局收束 + 可爱风 UI
...
1. 系统提示重写为"昆仑子——文化引路人"的人格：
  - 第一人称"我"，称呼玩家"你"，亲切、机敏、有温度（嗯 / 你看 / 唔 每段最多一次）。
...
1. 立绘资源仍未替换为正式的昆仑子立绘。当前 `CharacterSlot` 采用占位样式，
   待美术资产就位后再换文件名 / 语义槽即可。
2. UI 文案里"昆仑"既是世界名又是角色名，后续如果继续调整"昆仑子"
  的称呼，需要同时更新 `storyPromptBuilder` 的人设和 DialogPanel
   名字牌默认值。
...
2. **人设只守前两句**：昆仑子语气只在开头一两段生效，后面迅速退化为百科条目。
